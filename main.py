...
def collisions(player, obstacles):
    ...
    if obstacles:
        for i, obstacle_rect in enumerate(obstacles):
            if player.colliderect(obstacle_rect) and player.bottom < obstacle_rect.centery:
...
obstacle_types = [bee_surf, spider_surf,]
...
def spawn_obstacle():
    ...
    max_attempts = 5
    attempts = 0
    while any(obstacle_rect.colliderect(rect) for rect in obstacle_rect_list) and attempts < max_attempts:
        lane = choice(lanes)
        obstacle_type = choices(obstacle_types, cum_weights=obstacle_spawn_rates, k=1)[0]
        obstacle_width = obstacle_type.get_width()
        obstacle_height = obstacle_type.get_height()
...
while True:
    ...
    if game_active:
        ...
        if player_rect.bottom >= 916: player_rect.bottom = 916
        player_animation()
 
        screen.blit(player_surf , player_rect)
        obstacle_rect_list = obstacle_movement(obstacle_rect_list)
        game_active = collisions(player_rect, obstacle_rect_list)
        obstacle_rect_list = [obstacle for i, obstacle in enumerate(obstacle_rect_list) if i not in obstacles_to_remove]
        obstacles_to_remove.clear()
    else:
        screen.blit(gameover_surf, (0, 0))
        screen.blit(therun_surf, therun_rect)
        score_message = font.render('Score: ' + f'{(int(current_time / 1000) + score_multiplier * 2)}', False, 'Black')
